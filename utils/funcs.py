...
def plot_graph(start_point, length, target, inference,hr,epoch,ppg=True):
    if ppg:
        ...
        plt.savefig("result_ppg{}.jpg".format(epoch))
        plt.clf()
    else:
        plt.rcParams["figure.figsize"] = (14, 5)
        plt.plot(range(len(target[start_point:start_point + length])), target[start_point:start_point + length],
                 label='ppg_hr-bpm')
        plt.plot(range(len(inference[start_point:start_point + length])), inference[start_point:start_point + length],
                 label='ppg_hr_prediction-bpm')
        plt.plot(range(len(inference[start_point:start_point + length])), hr[start_point:start_point + length],
                 label='gt_hr-bpm')
        plt.legend(fontsize='x-large',loc='upper right')
        ...
        plt.savefig("result_bpm{}.jpg".format(epoch))
        plt.clf()
