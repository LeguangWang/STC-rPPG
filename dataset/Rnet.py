...
class RnetDataset(Dataset):
    def __init__(self, video_data, label_data,label_HR):
        self.transform = transforms.Compose([transforms.ToTensor()])
        self.video_data = video_data
        self.label = label_data
        self.label_HR = label_HR
    ...
    def __getitem__(self, index):
        ...
        label_HR = torch.tensor(self.label_HR[index], dtype=torch.float32)

        if torch.cuda.is_available():
            video_data = video_data.to('cuda')
            label_data = label_data.to('cuda')
            label_HR = label_HR.to('cuda')
        targets = torch.stack([label_data,label_HR],dim=0)
